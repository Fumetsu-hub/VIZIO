    ...
    <table  style="background-color:#FFF; font-size:20px; " class="table table-striped table-hover">
        ...
        <?php
            ...
            if(isset($prenom) or isset($nom) or isset($sexe)) // si une recherche est effectuée :
            {
              ...
              if(isset($prenom) and isset($nom) and isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE nom LIKE '%$nom%' AND prenom LIKE '%$prenom%'";echo"nom et prenom";}
              // recherche par nom et prenom
              else if(isset($prenom) and isset($nom) and !isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE nom LIKE '%$nom%' AND prenom LIKE '%$prenom%'";echo"nom et prenom";}
              // recherche par prenom
              else if(isset($prenom) and !isset($nom) and !isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE prenom LIKE '%$prenom%'";echo"prenom";}
              // recherche par nom
              else if(!isset($prenom) and isset($nom) and !isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE nom LIKE '%$nom%'";echo"nom";}
              // recherche par sexe
              else if(!isset($prenom) and !isset($nom) and isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE sexe='$sexe'";echo"sexe";}
              // recherche par sexe et prenom
              else if(isset($prenom) and !isset($nom) and isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE sexe='$sexe' AND prenom LIKE '%$prenom%'";echo"sexe et prenom";}
              // recherche par sexe et nom
              else if(!isset($prenom) and isset($nom) and isset($sexe)){$sel_query="SELECT id, nom, prenom, mail, date_n, sexe, tel ,statutFROM user WHERE sexe='$sexe' AND prenom LIKE '%$nom%'";echo"sexe et nom";}
